...
size_t byte_compress(
    ...
    // Initial setup for first item in the struct array
    ...
    // First pass; Will load all (count, value) pairs into the Struct array
    ...
    // Second pass; Compresses into the original buffer here after size checks
...
size_t byte_decompress(
    ...
    if (buff_ptr[0] != 0xFF) return buff_size;
    // Number of values is based on the 2n+1 format the compressor outputs for 
    // valid compressions
    ...
    // Check to make sure writing into the buffer wont exceed the allocated 
    // capacity based on the passed arg
    if (buff_capacity < decompressed_size) {
        ...
        unsigned char value;
        size_t i = 0;
    // Second pass; Decompresses into the original buffer here after size checks
        for (size_t j = 0; j < total_value_count; j++) {
